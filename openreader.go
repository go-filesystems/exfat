...
// OpenReader opens the exFAT filesystem that starts at offset zero of r.
//
...
func OpenReader(r io.ReaderAt, size int64) (filesystem.Filesystem, error) {
	if r == nil {
		return nil, errors.New("exfat: OpenReader needs a reader")
	}
	if size <= 0 {
		return nil, fmt.Errorf("exfat: an image of %d bytes is not one", size)
	}
	return openAt(readOnlyOrWritable(r), 0)
}

// ErrReadOnlyReader is what a write gets when the filesystem was opened over
// something that can only be read.
var ErrReadOnlyReader = errors.New("exfat: this filesystem was opened over a reader that cannot be written")
